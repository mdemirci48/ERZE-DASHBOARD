...
async function fetchLiveTurnover() {
    ...
    try {
        ...
        liveTurnover = data[1] || 0; // Assuming branch 1 is IZMIR

        // We will call a new function to update the table here
...
async function fetchLiveRawMaterials() {
    ...
    try {
        const response = await fetch('/api/live-raw-materials');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        ...
        tableTitle.textContent = `Raw Materials Live Cost - IZMIR for ${monthNames[currentMonth]}`;
...
async function fetchLiveExpenses() {
    ...
    try {
        const response = await fetch('/api/live-expenses');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        
        // Update table title with the current month
        const monthNames = ["January", "February", "March", "April", "May", "June",
                            "July", "August", "September", "October", "November", "December"];
        const currentMonth = new Date().getMonth();
...
function populateLiveExpenseTable(data) {
    ...
    data.forEach(item => {
        ...
        // Apply classes for styling subtotals and grand totals
...
function populateRawMaterialsTable(data) {
    ...
    const xpetTrayRow = Array.from(tbody.querySelectorAll('tr')).find(row => row.cells[0].textContent.trim() === 'XPET TRAY PURCHASES');
...
function populateTurnoverAndProfit() {
    ...
    const totalCost = parseFloat(totalsRow.cells[1].textContent.replace(/[^0-9,-]+/g,"").replace(',','.'));
...
async function fetchLiveTurnoverPrev() {
    ...
    try {
        ...
        const data = await response.json();
        liveTurnoverPrev = data[1] || 0; // Assuming branch 1 is IZMIR
...
async function fetchLiveRawMaterialsPrev() {
    ...
    try {
        const response = await fetch('/api/live-raw-materials-prev');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        ...
        tableTitle.textContent = `Raw Materials Live Cost - IZMIR for ${monthNames[prevMonth]}`;
...
async function fetchLiveExpensesPrev() {
    ...
    try {
        const response = await fetch('/api/live-expenses-prev');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
