...
async function fetchLiveTurnoverPrev() {
    ...
    try {
        ...
        const data = await response.json();
        liveTurnoverPrev = data[4] || 0; // FLEX is branch 4
...
async function fetchLiveRawMaterialsPrev() {
    ...
    try {
        const response = await fetch('/api/live-raw-materials-flex-prev');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        ...
        tableTitle.textContent = `Raw Materials Live Cost - FLEX for ${monthNames[prevMonth]}`;
...
async function fetchLiveExpensesPrev() {
    ...
    try {
        const response = await fetch('/api/live-expenses-flex-prev');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
