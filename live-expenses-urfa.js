...
async function fetchLiveTurnover() {
    ...
    try {
        ...
        liveTurnover = data[3] || 0; // URFA is branch 3
...
async function fetchLiveRawMaterials() {
    ...
    try {
        const response = await fetch('/api/live-raw-materials-urfa');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        ...
        tableTitle.textContent = `Raw Materials Live Cost - URFA for ${monthNames[currentMonth]}`;
...
async function fetchLiveExpenses() {
    ...
    try {
        const response = await fetch('/api/live-expenses-urfa');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
